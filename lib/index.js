...
export default {
  ...
  init(userOptions) {
    ...
    this.client = new Nodemw(options);
    this.client.getPagesInCategory = pify(
      this.client.getPagesInCategory.bind(this.client)
    );
  },
  ...
  async pagesFromCategory(categoryName) {
    const raw = await this.client.getPagesInCategory(categoryName);
    return _.map(raw, 'title');
  },
};
