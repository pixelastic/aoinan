...
describe('aoi', () => {
  ...
  let serverUrl;
  beforeEach(async () => {
    await firost.emptyDir(module.cacheLocation);
    if (serverStarted) {
      return;
    }
  ...
  describe('page', () => {
    ...
    it('should write to cache on first call', async () => {
      ...
      const actual = await firost.read(`${module.cacheLocation}/Foo`);

      expect(actual).toEqual('Foo content');
    });
    it('should read from cache if exists', async () => {
      await firost.write('cache', `${module.cacheLocation}/Foo`);
